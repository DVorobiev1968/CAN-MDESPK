...
/*****************************************************************************/
void MDESPKSlave_heartbeatError(CO_Data* d, UNS8 heartbeatID)
{
	eprintf("MDESPKSlave_heartbeatError %d\n", heartbeatID);
}

void MDESPKSlave_initialisation(CO_Data* d)
{
	eprintf("MDESPKSlave_initialisation\n");
}

void MDESPKSlave_preOperational(CO_Data* d)
{
	eprintf("MDESPKSlave_preOperational\n");
}

void MDESPKSlave_operational(CO_Data* d)
{
	eprintf("MDESPKSlave_operational\n");
}

void MDESPKSlave_stopped(CO_Data* d)
{
	eprintf("MDESPKSlave_stopped\n");
}

void MDESPKSlave_post_sync(CO_Data* d)
{
      eprintf("MDESPKSlave_post_sync\n");
...
{
        SlaveMap13 += 1;
	eprintf("MDESPKSlave_post_TPDO\n");
	
	/* send an error and recover inmediately every 12 cycles */
	if(SlaveMap13 % 12 == 0)
	{
		EMCY_setError(&MDESPKSlave_Data, 0x4200, 0x08, 0x0000);
		EMCY_errorRecovered(&MDESPKSlave_Data, 0x4200);
	}
}
...
void MDESPKSlave_storeODSubIndex(CO_Data* d, UNS16 wIndex, UNS8 bSubindex)
{
	 ...
	 * 
	 * */
	eprintf("MDESPKSlave_storeODSubIndex : %4.4x %2.2x\n", wIndex,  bSubindex);
}
