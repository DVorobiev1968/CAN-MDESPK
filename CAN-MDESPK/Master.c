...
/*****************************************************************************/
void MDESPKMaster_heartbeatError(CO_Data* d, UNS8 heartbeatID)
{
	eprintf("MDESPKMaster_heartbeatError %d\n", heartbeatID);
}
...
void MDESPKMaster_initialisation(CO_Data* d)
{
	...
	eprintf("MDESPKMaster_initialisation\n");

	/*****************************************
	 * Define RPDOs to match slave ID=2 TPDOs*
	 *****************************************/
	writeLocalDict( &MDESPKMaster_Data, /*CO_Data* d*/
			0x1400, /*UNS16 index*/
			0x01, /*UNS8 subind*/ 
...
static void CheckSDOAndContinue(CO_Data* d, UNS8 nodeId)
{
	...
	closeSDOtransfer(&MDESPKMaster_Data, nodeId, SDO_CLIENT);
 ...
 ********************************************************
 * This is called first by MDESPKMaster_post_SlaveBootup
...
{
	eprintf("MDESPKMaster_preOperational\n");
}

void MDESPKMaster_operational(CO_Data* d)
{
	eprintf("MDESPKMaster_operational\n");
}

void MDESPKMaster_stopped(CO_Data* d)
{
	eprintf("MDESPKMaster_stopped\n");
}

void MDESPKMaster_post_sync(CO_Data* d)
{
	eprintf("MDESPKMaster_post_sync\n");
...
{
	eprintf("MDESPKMaster_post_TPDO MasterSyncCount = %d \n", MasterSyncCount);
	if(MasterSyncCount % 17 == 0){
		eprintf("Master : Ask RTR PDO (0x1402)\n");
		sendPDOrequest(&MDESPKMaster_Data, 0x1402 );
		sendPDOrequest(&MDESPKMaster_Data, 0x1403 );
	}
	if(MasterSyncCount % 50 == 0){
		UNS8 transmitiontype;
		eprintf("Master : Change slave's transmit type to 0xFF\n");
		transmitiontype = 0xFF;
		writeNetworkDictCallBack (&MDESPKMaster_Data, /*CO_Data* d*/
					2, /*UNS8 nodeId*/
					0x1802, /*UNS16 index*/
					0x02, /*UNS16 index*/
					1, /*UNS8 count*/
...
{
	eprintf("MDESPKMaster_post_SlaveBootup %x\n", nodeid);
